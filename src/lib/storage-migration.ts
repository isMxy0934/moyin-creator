...
export async function migrateToProjectStorage(): Promise<void> {
  ...
  try {
    ...
    const projectStoreRaw = await fileStorage.getItem('mumu-project-store');
    if (!projectStoreRaw) {
    ...
    await migrateRecordStore('mumu-script-store', 'script', projectIds);
    await migrateRecordStore('mumu-director-store', 'director', projectIds);

    // 3. Migrate flat-array stores (media, characters, scenes)
    await migrateFlatStore('mumu-media-store', 'media', projectIds, {
    ...
    await migrateFlatStore('mumu-character-library', 'characters', projectIds, {
      arrayKeys: ['characters', 'folders'],
      projectIdField: 'projectId',
      sharedFilter: (item: any) => !item.projectId,
    });

    await migrateFlatStore('mumu-scene-store', 'scenes', projectIds, {
...
async function migrateTimelineStore(activeProjectId: string): Promise<void> {
  const raw = await fileStorage.getItem('mumu-timeline-store');
  if (!raw) return;
...
export async function recoverFromLegacy(): Promise<void> {
  ...
  try {
    ...
    await recoverRecordStore('mumu-script-store', 'script', isScriptDataRich);
    await recoverRecordStore('mumu-director-store', 'director', isDirectorDataRich);
